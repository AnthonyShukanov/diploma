...
const { base, login, github, dashboard, api, profile } = require('../support/po/PageMap')
...
describe('Тестирование основного функционала профиля пользователя', () => {
    ...
    it('2.7 - Алгоритм проверки данных на странице пользователя', () => {
        cy.log('Проверка на совпадение адреса после перехода')
        dashboard.url().should('include', '/launches')
        cy.log('Нажатие на кнопку "Profile"')
        dashboard.profileButton.click()
        cy.log('Проверка на существование кнопки "Profile"')
        dashboard.mainProfileButton.should('be.visible')
        cy.log('Нажатие на кнопку "Profile"')
        dashboard.mainProfileButton.click()
        cy.log('Проверка на существование кнопки "Change password"')
        profile.changePasswordButton.should('be.visible')
    })
    it('2.8 - Алгоритм проверки возможности пользователя просмотра API', () => {
        cy.log('Проверка на совпадение адреса после перехода')
        dashboard.url().should('include', '/launches')
        cy.log('Нажатие на кнопку "Profile"')
        dashboard.profileButton.click()
        cy.log('Проверка на существование кнопки "API"')
        dashboard.apiButton.should('be.visible')
        cy.log('Нажатие на кнопку "API"')
        dashboard.apiButton.click()
        cy.log('Проверка на существование кнопки "Show/hide"')
        api.showButton.should('be.visible')
    })
})